...
class DebuggerRunner(object):
    ...
    def check_case_simple(self, target, test_file):
        ...
        with self.check_case(WriterThreadCase) as writer_thread:
            yield writer_thread
    ...
    def check_case(self, writer_thread_class):
        ...
        try:
            ...
            with self.run_process(args, writer_thread) as dct_with_stdout_stder:
                for _i in xrange(40000):
                    if writer_thread.finished_initialization:
                        break
                    time.sleep(.01)
                yield writer_thread
        finally:
